#!/usr/bin/env bash
# Extract ONLY the augmented OOD data (data/JEPA + data/MULTI) into the existing
# states_wang dirs, layer -1 + layer 25, all 10 cells. Skips Wang's RS1/RS2/RS3
# entirely (those are already extracted). Use after extract_wang_full.sh.
set -euo pipefail
...
LOG_DIR=$REPO/runs/experiments/headline_backup_rerun_probing/extract_wang_aug_logs
mkdir -p "$LOG_DIR"
...
wait
echo "All Wang-aug extractions complete."
