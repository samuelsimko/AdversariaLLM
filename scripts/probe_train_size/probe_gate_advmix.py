"""WJ-only probe with mixed vanilla + adversarial styles on both sides.

Positives (harmful): N/2 vanilla harmful + N/2 adversarial harmful
Negatives (benign):  N/2 vanilla benign  + N/2 adversarial benign

Default N=3000 per class. No WildGuardMix, no UltraChat — just clean WJ.
This isolates the question: "does adding adversarial-wrapper styles to *both*
sides during training change the probe's runtime gating?"
"""
...
def sample_advmix(n_per_class: int, seed: int = 42) -> tuple[list[str], list[str]]:
    """50/50 split: vanilla + adversarial WJ for both classes."""
    random.seed(seed)
    half = n_per_class // 2
    ...
    for L in (h_vanilla, h_adv, b_vanilla, b_adv):
        random.shuffle(L)

    pos = h_vanilla[:half] + h_adv[:half]
    neg = b_vanilla[:half] + b_adv[:half]
    print(f"[train pool] pos: {half} vanilla harmful + {half} adversarial harmful = {len(pos)}")
    print(f"[train pool] neg: {half} vanilla benign  + {half} adversarial benign  = {len(neg)}")
...
def main():
    ...
    ap.add_argument("--out_dir", type=Path,
                    default=REPO / "runs/experiments/headline_backup_rerun_probing/probe_gate_advmix")
    ap.add_argument("--seed", type=int, default=42)
    args = ap.parse_args()

    args.out_dir.mkdir(parents=True, exist_ok=True)
    print(f"\n{'='*60}\nCell: {args.cell}  (advmix {args.n_train}/class)\n{'='*60}", flush=True)

    eval_rows = []
    for atk in args.attacks:
        rows = iter_attack(args.cell, atk, args.max_per_attack)
        if rows: print(f"  {atk:<22} {len(rows)} prompts (asr={sum(r['success'] for r in rows)}/{len(rows)})")
    ...
    h_p, b_p = sample_advmix(args.n_train, seed=args.seed)
