...
def main() -> None:
    ...
    # Persist the trained probe so downstream scripts (probe-gated ASR analysis)
    # can apply the same scaler+SVM to attack/benign reps without retraining.
    import joblib
    joblib_path = args.out_csv.with_suffix(".joblib")
    joblib.dump(
        {"scaler": scaler, "svm": svm, "cell": args.cell, "layer": args.layer,
         "id_stems": list(id_stems), "predictor_path": str(args.predictor_path) if args.predictor_path else None,
         "predictor_layers": args.predictor_layers, "predictor_bottleneck_dim": args.predictor_bottleneck_dim},
        joblib_path,
    )
