"""advmix probe with heavy benign anchor (3000 UltraChat in negatives).

Positives (3000): 1500 WJ vanilla harmful + 1500 WJ adversarial harmful
Negatives (5000): 1000 WJ vanilla benign + 1000 WJ adversarial benign + 3000 UltraChat

The class imbalance (3000 pos / 5000 neg) is intentional: we want the probe
to be biased toward "this looks like normal chat" so it false-positives less
on WildChat at runtime. Linear SVM with `class_weight='balanced'` to keep
margins fair despite the imbalance.
"""
...
def sample_train(n_pos: int = 3000, n_uc: int = 3000, n_wj_neg: int = 1000, seed: int = 42):
    ...
    b_uc = [uc[i]["prompt"] for i in range(min(50000, len(uc)))]

    for L in (h_vanilla, h_adv, b_vanilla, b_adv, b_uc):
        random.shuffle(L)

    pos = h_vanilla[:pos_half] + h_adv[:pos_half]
    neg = b_vanilla[:n_wj_neg] + b_adv[:n_wj_neg] + b_uc[:n_uc]
    print(f"[train pool] pos={len(pos)} ({pos_half} vanilla + {pos_half} adv harmful)")
    print(f"[train pool] neg={len(neg)} ({n_wj_neg} WJ vanilla + {n_wj_neg} WJ adv + {n_uc} UltraChat)")
...
def sample_wildchat(n, seed=42):
    """OOD eval source — different seed from training to avoid overlap."""
...
def fpr_asr_table(df, probe_col, fprs=(0.005, 0.01, 0.02, 0.05, 0.10, 0.20)):
...
def main():
    ...
    ap.add_argument("--n_pos", type=int, default=3000)
    ap.add_argument("--n_uc", type=int, default=3000)
    ap.add_argument("--n_wj_neg", type=int, default=1000)
    ...
    ap.add_argument("--out_dir", type=Path,
                    default=REPO / "runs/experiments/headline_backup_rerun_probing/probe_gate_heavy_benign")
    ap.add_argument("--seed", type=int, default=42)
    args = ap.parse_args()

    args.out_dir.mkdir(parents=True, exist_ok=True)
    print(f"\n{'='*60}\nCell: {args.cell} (heavy-benign: pos={args.n_pos}, neg={args.n_uc + 2*args.n_wj_neg})\n{'='*60}", flush=True)
    ...
    h_p, b_p = sample_train(args.n_pos, args.n_uc, args.n_wj_neg, seed=args.seed)
    ...
    # class_weight balanced because the negatives outnumber positives
    svm = SVC(kernel="linear", random_state=args.seed, probability=True,
              class_weight="balanced").fit(scaler.transform(X_train), y_train)
    ...
    X_eval = extract_reps(model, tok, [r["prompt"] for r in eval_rows])
    df = pd.DataFrame(eval_rows)
    df["probe_score"] = svm.predict_proba(scaler.transform(X_eval))[:, 1]

    df.to_csv(args.out_dir / f"{args.cell}.csv", index=False)
    joblib.dump({"scaler": scaler, "svm": svm, "cell": args.cell,
                 "n_pos": args.n_pos, "n_uc": args.n_uc, "n_wj_neg": args.n_wj_neg},
