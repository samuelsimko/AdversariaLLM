...
class LocalGPUBackend(Backend):
    ...
    def _run_job(self, worker_id: int, gpu_id: str, job: _Job, env: Dict[str, str]):
        ...
        if self.venv_activate and Path(self.venv_activate).exists():
            wrapped_cmd += f"source {shlex.quote(self.venv_activate)}; "
        if self.env_file and Path(self.env_file).exists():
            wrapped_cmd += f"source {shlex.quote(self.env_file)}; "
        wrapped_cmd += "nvidia-smi; sleep 1; " + shlex.join(job.command)
